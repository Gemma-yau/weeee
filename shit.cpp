...
bool canconnect(int next, vector< vector<int> > lst, coord present,string &status,vector<coord >used){
  int lent=used.size();
 ...
 if (present.first == previous.first){
   ...
   return false;}
      ...
      int seq[16]={1,2,3,4,3,2,1,2,3,4,3,2,1,2,3,4};
	...
	return; //what to do? or print "u cant move like that"
	 ...
	 if (present.first==3){
      ...
      //create a previous point,so if present and previous lie on same line-> dont allow
       if (canconnect(next,lst,present,status,used)){
